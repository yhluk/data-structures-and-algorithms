...
// resultTest.push(ingArr[i].substr(ingArr[i].indexOf(" ") + 1))
//   }
//   for (let i = 0; i < resultTest.length; i++){
// result.push(resultTest[i].substr(resultTest[i].indexOf(" ") + 1))
//   }
  ...
  for (let i = 0; i < ingArr.length; i++)
  {
    result.push(ingArr[i].substr(ingArr[i].indexOf(" ", 5) + 1))
  }
...
const splitFoods = (recipe) => {
  ...
  result.push(ingArr[i].split(' ').slice(2).join(' '));
  }
  return result;
};
...
const stepActions = (recipe) => {
  ...
  {
  result.push(stepsArr[i].split(' ').slice(0,1).join(' '));
  }
  return result;
};
