...
const divisibleByFiveTwoToThePower = (input) => {
  let filteredArr = input.map(valIn => valIn
    .filter(valIn => (typeof valIn === 'number' && valIn % 5 === 0)))
  return filteredArr.map(inArr => inArr
...
let findMaleAndFemale = (data) => {
  let namesArr = []
  data.forEach(obj => obj.gender === 'female' || obj.gender === 'male' ? namesArr.push(obj.name) : namesArr)
...
let findShortest = (data) => {
  const name = data.reduce(
    (acc, obj) =>
      Number(acc.height) < Number(obj.height) ?
        acc :
        obj
  );
  console.log(name)
