...
const countNumberOfChildren = (arr) => {
  return arr.reduce((acc, obj) => obj.children ? acc + obj.children.length : acc, 0);
};
...
const extractStats = (snorlaxData) => {
  let statsArray = snorlaxData.stats;
  return newArray.reduce((acc, val) =>
  acc[val.stat.name] = val.baseStat, {}
  )
  console.log('daaaaaaaataaaaaaaaaaaaa', statsArray.baseStat);
};
