...
const sortByChildren = (charArray) => {
  ...
  let arr1 = charArray.sort((a, b) => a.children.length - b.children.length)
  // console.log(arr1)
  arr1.sort((a, b) => a.children.length === b.children.length? -1 : 0)
  // console.log(arr1)
...
const citiesAtoJ = (arr) => {
  let regex = /^[A-Ja-j]/;
  const regArr = arr.filter(val => regex.test(val));
  return regArr;
};
