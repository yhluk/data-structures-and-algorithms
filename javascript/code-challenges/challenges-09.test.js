...
const maxInArray = (arr) => {
  return arr.reduce((acc, val) => val > acc? acc=val : acc, '')
};
...
const checkValues = (obj, val) => {
  return Object.values(obj).includes(val)
  // every(val => val === undefined ? true : false)
  // return first.map(val => val !== undefined ? false : false)
};
...
const getHouses = (arr) => {
  let houses = [];
    for (let i =0; i < arr.length; i++){
      Object.entries(arr[i]).map(([key, value]) => {
        key  === 'house' ? houses.push(value) : houses ;
      });
      // console.log(houses)
    }
  return houses;
...
const totalCharacters = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    for (let i = 0; i < arr[i].children.length; i++){
      charNum += 1
      console.log(charNum)
