...
const getStatName = (arr, minBaseStat) => {
  let arr07 = arr.filter(obj => obj.baseStat > minBaseStat)
  return arr07.map(obj => obj.stat.name)
};
...
const evenOddNumericValues = (arr) => {
  let firstArr = arr.filter(val => (typeof(val) !== 'string'));
  return firstArr.map((val) =>
  {
    if (val % 2 === 0)
    {
      return 'even';
